...
namespace Capstone.Web.Classes
{
    public class HashSalt
    {
        ...
        static Password HashPasswordWithPBKDF2(string password, int saltSize = 8, int workFactor = 1)
        {
            ...
            string needsSplit = Convert.ToBase64String(salt) + "|" + Convert.ToBase64String(hash);
            saltArray = needsSplit.Split('|');
            p.UnSaltedPassword = saltArray[0];
            p.Salt = saltArray[1];
